...
def _get_lr_scheduler(args, kv):
    ...
    logging.info(step_epochs)
    logging.info(begin_epoch)
    logging.info(steps)
...
def add_fit_args(parser):
    ...
    train.add_argument('--work-id', type=int, default = 0)
    return train
...
def fit(args, network, data_loader, **kwargs):
    ...
    if args.test_io:
        ...
        for i, batch in enumerate(train):
            for j in batch.data:
                j.wait_to_read()
            #if (i+1) % args.disp_batches == 0 and 
            if args.num == 0:
                logging.info('Batch [%d]\tSpeed: %.5f samples/sec' %(
                    i, args.disp_batches*args.batch_size/(time.time()-tic) * args.nwk))
                tic = time.time()
